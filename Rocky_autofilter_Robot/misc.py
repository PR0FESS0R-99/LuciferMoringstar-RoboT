...
async def imdb_callback(bot: Client, query: CallbackQuery):
    ...
    if imdb.get('poster'):
        await query.message.reply_photo(photo=imdb['poster'], caption=f"<b>{query} : {title} [{year}]

🌟 Rᴀᴛɪɴɢ : {rating}
💀 Rᴇʟᴇᴀsᴇ : {release_date}

‣ Lᴏᴄᴀʟ : {local}
‣ Gᴇɴʀᴇ : {genres}</b>", reply_markup=InlineKeyboardMarkup(btn))
        await query.message.delete()
    else:
        await query.message.edit(f"<b>{query} : {title} [{year}]

🌟 Rᴀᴛɪɴɢ : {rating}
💀 Rᴇʟᴇᴀsᴇ : {release_date}

‣ Lᴏᴄᴀʟ : {local}
‣ Gᴇɴʀᴇ : {genres}</b>", reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True)
    await query.answer()
