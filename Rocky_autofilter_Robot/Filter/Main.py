    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                filename = f"{file.file_name}"
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}", callback_url=f"https://telegram.dog/{nyva}?start=sachin9742s_-_-_-_{file_id}"),
                     InlineKeyboardButton(text=f"[{get_size(file.file_size)}]", callback_url=f"https://telegram.dog/{nyva}?start=sachin9742s_-_-_-_{file_id}")]
                )
        else:
            return
        if not btn:
